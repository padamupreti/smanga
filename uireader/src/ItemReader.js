...
const ItemReader = (props) => {
    ...
    useEffect(() => {
        if(data)
            document.title = `${data.name} | ${itemTitle} ${itemNum}`
    }, [data, itemTitle, itemNum])

    useEffect(() => {
        const handleKeyPress = (e) => {
            if(e.code === 'Space' || e.code === 'ArrowLeft' || e.code === 'ArrowDown') {
    ...
    const handleSliderInput = (e) => setVisibilityIndex(parseInt(e.target.value, 10) - 1)
